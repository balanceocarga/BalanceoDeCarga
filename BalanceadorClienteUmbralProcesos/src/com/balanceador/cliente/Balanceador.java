	...
	public static void main(String args[]) throws SQLException, RemoteException, InterruptedException, UnknownHostException{
		...
		while(itServidores.hasNext()){
			...
			bindingProvider.getRequestContext().put(
				      ...
				      "http://"+itServidores.next()+":8080/BalanceadorServidorUmbralProcesos/Balanceador");
	    ...
	    while (itProcesos.hasNext())
	    {
	        ...
	        for (i = 0; i < listaIPs.size(); i++)
	        {
	            ...
	            BindingProvider bindingProvider = (BindingProvider) cpu;
	            int cantProcesos = prop.obtenerPropiedad(((String) bindingProvider.getRequestContext().get(BindingProvider.ENDPOINT_ADDRESS_PROPERTY)).substring(7, 19));
	            if (cantProcesos <= 5)
	            {
	            	System.out.println("Procesando en servidor : " + p +" "+ cpu.toString());
		            ArrayList lista = new ArrayList();
		            lista.add(p);
		            prop.escribirPropiedad(((String) bindingProvider.getRequestContext().get(BindingProvider.ENDPOINT_ADDRESS_PROPERTY)).substring(7, 19));
		            cpu.ejecutarProceso(lista);
		            ejecutado = true;
	            }
	            if(i==proxyServidores.length-1 && ejecutado==false){
